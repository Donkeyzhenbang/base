...
//! 这里第三个地方加上typename是告诉编译器这里是类型名字不是静态变量
...
void heap_sort(RandomIt first, RandomIt last, Compare comp = Compare())
{
    ...
    if(n <= 1) return;
    auto heapify = [&](auto idx, auto heapSize) ->void {
        while(true){
            auto largest = idx;
            diff_t l = 2 * idx + 1;
            diff_t r = 2 * idx + 2;
            if(l < heapSize && comp(*(first + largest), *(first + l)))
                largest = l;
            if(r < largest && comp(*(first + largest), *(first + r)))
                largest = r;
...
private:
    // 这里只是实例化不是vec套vec T 后续是vector<int>那是给PrintObserver的notify方法使用的
...
struct SortTask{
    ...
    std::function<void(std::shared_ptr<std::vector<T>>)> callback;
    // optional per-task callback
};
...
void worker_loop(ThreadSafeQueue<std::shared_ptr<SortTask<T>>>& q, Subject<std::vector<T>>& subject, std::atomic<bool>& running) {
    while(running){
        ...
        // subject对象存的就是observer
        //! 所以设计时不必要追求设想太过长远，先做好当下功能即可,很多复杂的设计都可以化繁为简，
        //! 通过接口和容器的改变达到功能，重要的是保障业务功能稳定可行性和扩展性。
